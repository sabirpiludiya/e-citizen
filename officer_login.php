...
{	
		...
		$stmt = $con->prepare("SELECT * FROM `Tbl_officer` where `o_mail` = '$_POST[txt_mail]'");
		$stmt->execute();
	...
	{
		$err_pwd = "Mail or Password is Incorrect.";
		$count++;	
	}
...
if(isset($_POST['btn_submit']) && $count==0)
{
		...
		<?php
	if ($plain == $_POST['txt_pwd'] && $mailr == $_POST['txt_mail'] && $statr == 1)
	{
		?> <script>
				alert("In CHeck");
			</script>		
		...
		$_SESSION['officer_mail'] = $mail;
		$_SESSION['officer_type'] = $typer;
...
<html>
	<head>
		<title>Officer Login|| E Officer</title>
				...
				<div style="padding:5%;margin:0 auto;width:100%;text-align:left" align="center">
                	...
                	<form id="registerform" method="post" name="registerform" enctype="multipart/form-data" class="content">
							...
							<div class="form-group">
									...
									<input type="text" class="form-control" onKeyPress="return hide(event)" name="txt_mail" value="<?php echo $mail; ?>" placeholder="Enter Your Mail">
								<div class="notice_add"  id="mail_show_hide" align="left"><?php echo $err_mail; ?></div>
								</div><br/>					
								<div class="form-group">
