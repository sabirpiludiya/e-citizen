...
include "connection.php";
	...
	if(isset($_GET['reject_ic']))
	{
		$stmt = $con->prepare("delete from `Tbl_income_cert` where `ic_id` = '$_GET[reject_ic]'");
		$stmt->execute();
	}

	if(isset($_GET['approve_ic']))
	{
		$stmt = $con->prepare("update `Tbl_income_cert` set `ic_status` = 2 where `ic_id` = '$_GET[approve_ic]'");
		$stmt->execute();
	}
			...
			<!-- Banner -->
				...
				<div style="padding:5%;margin:0 auto;width:100%;text-align:left" align="center">
                	...
                	<form id="registerform" method="post" name="registerform" enctype="multipart/form-data" class="content">
									...
									<select id="ddl_certificate">
										<option value="--Select Certificate--" selected="selected"><label><b>--Select Certificate--</b></label></option>
										<option value="Income"><label><b>Income Certificate</b></label></option>
										<option value="SCST"><label><b>SCST Certificate</b></label></option>
										<option value="SEBC"><label><b>SEBC Certificate</b></label></option>
										<option value="NonCreamyLayer"><label><b>Non-CreamyLayer Certificate</b></label></option>
										<option value="SeniorCitizen"><label><b>Senior Citizen Certificate</b></label></option>
									</select>
					<br/><br/>
					
<SCRIPT> 	
$(document).ready(function () {
            $('#ddl_certificate').change(function () {
                if (this.value == "Income") {
                    $('#income').show(); }
                else if (this.value == "SCST") {
                    $('#scst').show(); }
                else if (this.value == "SEBC") {
                    $('#sebc').show(); }
                else if (this.value == "NonCreamyLayer") {
                    $('#noncreamylayer').show(); }
                else if (this.value == "SeniorCitizen") {
                    $('#seniorcitizen').show(); 
                } else {
                    $('#income').hide();
                    $('#scst').hide();
					...
					<div class="table-responsive" style="display: none"  id="seniorcitizen">
...
<?php
		$con = new PDO("mysql:host=$host;dbname=$db", $uname, $pass);
		$con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$stmt = $con->prepare("SELECT * FROM `Tbl_income_cert` a, `Tbl_user` b where `ic_aadhar_id` = `u_aadhar_id` and `ic_status` = 1");
		$stmt->execute();
		$c = 1;						
...
?>
							...
							<tr>
							  ...
							  <td>
			<?php echo $pwd = $rows ['sc_aadhar_id']; ?></td>
							  <td>
...
<!-- The Modal -->
 	<div id="myModal<?php echo $c; ?>" class="modal">
		  ...
		  <div class="modal-content" style="color:#000000">
			...
			<p align="right"><img src="<?php echo $rows['u_photo'] ; ?>" height="10%" width="10%"/></p>
...
?>						
<tr>
<td colspan="4">Sorry!!! No Certificate	 Requests..!</td>
</tr>						
<?php
}
?>						
